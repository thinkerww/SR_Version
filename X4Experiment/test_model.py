...
#                nn.Conv2d(512, out_channels=64, kernel_size=1,padding=0, bias=True),
    ...
    def forward(self,x):
        ...
        Input_x = F.interpolate(x, scale_factor=4, mode='bilinear', align_corners=False)
...
#            print(out2_1_R.shape)
            ...
            HR_2 = torch.cat((HR_2_con,Output_2),1)
            ...
            out4_1 = self.DB4_1(X4_Input)
            out4_1_B = self.DB4_1_B(out4_1)
            # 外边 局部的 residual link
            out4_1_B = torch.add(out4_1_B,X4_Input)
            
            out4_1_R,next_out4_1_B = self.GRU4_1(out4_1_B,R4_1)
            R4_1 = next_out4_1_B
            
            #第二个块 
            out4_2 = self.DB4_2(out4_1_R)
            out4_2_B = self.DB4_2_B(out4_2)
            # residual link
            out4_2_B = torch.add(out4_2_B,out4_1_R)
            
            out4_2_R,next_out4_2_B = self.GRU4_2(out4_2_B,R4_2)
            R4_2 = next_out4_2_B
            
            #第三个块 
            out4_3 = self.DB4_3(out4_2_R)
            out4_3_B = self.DB4_3_B(out4_3)
            # residual link
            out4_3_B = torch.add(out4_3_B,out4_2_R)
            
            out4_3_R,next_out4_3_B = self.GRU4_3(out4_3_B,R4_3)
            R4_3 = next_out4_3_B
            
            # 整合 上述的 RB块 
            x4_finally = torch.cat((out4_1_R,out4_2_R,out4_3_R),1)
            x4_finally = self.x4_DB_add(x4_finally)
            # 用 ConvGRU 保存
            x4_finally,next_x4_finally = self.x4_GRU_add(x4_finally,R4_6)
            R4_6 = next_x4_finally
            
            # 上采样 X4 图像 
            HR_4_con =self.HR4(x4_finally) 
            HR_4 = torch.cat((HR_4_con,next_input),1)
            # 作为 第二个阶段的 输入 X4阶段 128 通道 
        ...
        return HR0_list,HR2_list,HR4_list
